...
**סטטוס בדיקות:** 173/173 טסטים עוברים בהצלחה + `ruff check` נקי  
...
| **P6.2** | ⏳ בהמתנה | **מערכת תורים אסינכרונית (Job Queue + Progress Bar)** | ניהול באצ'ים כבדים ברקע עם מזהה `job_id` ובדיקת סטטוס אסינכרונית. | רינדור חלק של עשרות מוקאפים ללא חשש מ-HTTP Timeout. |
| **P6.3** | ⏳ בהמתנה | **חבילות ליסטינג אוטומטיות לחנויות (Etsy Listing Bundles)** | יצירת סט תמונות שלם למוצר בלחיצה אחת: Hero Lifestyle, תקריב, קנה מידה ביחס לחדר, ומדריך גדלים. | חיסכון של שעות עבודה ידניות בהכנת ליסטינגים לחנויות. |
...
3. **נריץ את מערך הטסטים (159 בדיקות)** כדי לוודא שאין שום רגרסיה ושהכל עובד בצורה מושלמת.
