...
**סטטוס בדיקות:** 187/187 טסטים עוברים בהצלחה + `ruff check` נקי  
...
| **P1.3** | ✅ **הושלם** | **הגבלת גודל העלאה ואבטחת סביבה** | `DEFAULT_MAX_CONTENT_LENGTH = 32MB` כברירת מחדל (משתנה סביבה גובר; `0` מבטל את התקרה); `create_app` מסרב לעלות עם `SECRET_KEY` הדיפולטי מחוץ ל-debug/testing; ו-Rate Limiting ללוגין — 10 ניסיונות לכתובת בחלון של 5 דקות, מתאפס בכניסה מוצלחת. |
...
| **P4.2** | ⏳ בהמתנה | **שילוב Real-ESRGAN כ-AI Upscaler אוטומטי** | אינטגרציה של `RealESRGAN_x4plus.pth` להגדלה וחידוד של יצירות אמנות ברזולוציה נמוכה שהועלו על ידי לקוחות. | מוקאפים באיכות הדפסה חדה (300 DPI) גם מתמונות מקור קטנות. |
...
| **P4.4** | ⏳ בהמתנה | **יצירת מוקאפים ב-AI מתוך הסטודיו (Vertex AI · Nano Banana 2)** | מסך ייעודי שמייצר תמונות מוקאפ חדשות במודל התמונות של Vertex AI, עם פרומפט שמכתיב מסך ירוק זרחני אחיד (chroma key) במקום היצירה — כך שהתוצר נכנס היישר למנוע זיהוי המסגרות הירוקות הקיים ונשמר כתבנית. כולל בקרת סצנה (חדר, סגנון, תאורה, יחס גובה-רוחב ומספר מסגרות), בדיקת איכות שמוודאת שהירוק אכן אחיד ובתוך טווח ה-tolerance לפני השמירה, וייבוא אוטומטי לקטלוג. | המשתמש כבר לא תלוי בייבוא מוקאפים מבחוץ — הוא מייצר קטלוג משלו מתוך הסטודיו. |
...
| **P6.3** | ✅ **הושלם** | **חבילות ליסטינג אוטומטיות לחנויות (Etsy Listing Bundles)** | `POST /api/mockups/listing-bundle` — יצירה אחת מייצרת את כל התמונות שליסטינג צריך: **hero** (התבנית המתאימה ביותר, בבחירה אוטומטית), **closeup** (חיתוך של תמונת ה-hero סביב המסגרת — לא רינדור שני, ולכן חינם ותמיד תואם לצילום שממנו נחתך), **scale** (תבנית *אחרת* מה-hero כדי שהליסטינג לא יציג פעמיים את אותה תמונה) ו-**size_guide** (טבלת גדלים מצוירת: מלבנים מקוננים עם תוויות, והיצירה עצמה מוטבעת בעדינות בגדול שבהם). משפחת הגדלים נבחרת לפי יחס הצדדים של היצירה (2:3, 3:4, 4:5, 5:7, ISO A, 1:1) ומסתובבת ליצירה רוחבית; אפשר גם לשלוח רשימת גדלים משלך. כל תפקיד נכשל לבד — תבנית שבורה עולה תמונה אחת, לא את כל הסט (207), ורק בקשה שגויה בעצמה נדחית ב-400. אגב כך נמצא ותוקן באג: קובץ `.avif` שנשמר לדיסק נכתב בפועל כ-JPEG. בסטודיו נוסף כפתור **Listing set** במסך Test mockups: הוא נדלק ברגע שיש יצירה על המסך (בלי לבחור תבניות — הוא בוחר בעצמו), מציג את ארבע התמונות באותן כרטיסיות של באצ' — ולכן ה-Lightbox ו-Download all עובדים עליהן מעצמם — וכל כרטיס מראה את שם התבנית שנבחרה. 15 בדיקות חדשות; אומת מקצה לקצה מול הקטלוג האמיתי — סט מלא ב-2.1 שניות דרך ה-API ודרך הדפדפן, בלי שגיאות קונסולה. | חיסכון של שעות עבודה ידניות בהכנת ליסטינגים לחנויות. |
...
3. **נריץ את מערך הטסטים (187 בדיקות)** כדי לוודא שאין שום רגרסיה ושהכל עובד בצורה מושלמת.
