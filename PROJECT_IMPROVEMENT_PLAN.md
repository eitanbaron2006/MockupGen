...
**סטטוס בדיקות:** 146/146 טסטים עוברים בהצלחה  
**עדכון אחרון:** 2026-08-26 — שלב 1 (אבטחה קריטית) הושלם במלואו  
...
| **P2.5** | ⏳ בהמתנה | **איחוד כפילויות לוגיות ב-Backend** | ריכוז לוגיקת יצירת `manifest.json` לפונקציה מרכזית אחת, איחוד ציור מסכות מפוליגונים, ואיחוד `json_error` מול `error_response`. | מניעת באגים הנובעים מסטיות במימושים מקבילים. |
...
| **P3.3** | ⏳ בהמתנה | **צמצום תפיסת הזיכרון ב-Green Detection Cache** | שמירת מסכות בינאריות/דחוסות בלבד ב-Cache במקום 5 מערכי NumPy ענקיים בגודל Canvas מלא (מונע זליגת זיכרון ב-6000x6000px). | חיסכון של מאות מגהבייטים/ג'יגהבייטים של RAM ברינדורים תכופים. |
| **P3.4** | ⏳ בהמתנה | **ThreadPoolExecutor גלובלי משותף** | הגדרת Worker Pool יחיד ברמת האפליקציה עבור Batch Detection במקום יצירת Pool חדש בכל בקשה. | מניעת הצפת משאבי מעבד וקריאות Vertex AI לא מבוקרות. |
...
3. **נריץ את מערך הטסטים (146 בדיקות)** כדי לוודא שאין שום רגרסיה ושהכל עובד בצורה מושלמת.
