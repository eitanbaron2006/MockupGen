...
| **P3.D** | ✅ **הושלם** | **התמדה: מה נשמר, מה לא, ולמה** | **בוצע 03–04.09.2026.** נסקר מה מחזיקה המערכת ומה נעלם ב-restart, והוכרע לפי שאלה אחת: *אם השרת ייפול עכשיו, מה כואב?* **נוסף ל-DB:** (א) **היסטוריית ייצוא הדפסה** — `exports` + `export_files` ב-DB של ההדפסה, נרשמת בכל ייצוא עם היצירה, הסט, המצב, האיכות והמשקל, עם `reference` שהאפליקציה החיצונית מזהה בו ליסטינג; `GET/DELETE /api/print/exports` ו-`retention_days` (30) שסוחף ייצואים שפג תוקפם **וגם קבצים שאף רשומה לא תובעת** — אלה שנכתבו לפני שההיסטוריה קיימת. (ב) **העדפות ה-UI** — `settings["UI_PREFERENCES"]`, מוגשות **עם ה-HTML** כי הסטודיו קורא אותן סינכרונית ב-12 מקומות; `localStorage` הפך למטמון בלבד וה-DB מנצח (נמדד: מטמון עם 999 מול DB עם 322 → הסטודיו לקח 322). (ג) **שגיאות** — `error_log` חסום ל-1000, נטען חזרה בעלייה כדי ש-Server Pulse לא ייפתח על רשימה ריקה שמשקרת. (ד) **ניסיונות כניסה** — `login_attempts`; קודם כל restart נתן למנחש עשרה ניסיונות חדשים. **נשאר בזיכרון בכוונה:** בקשות ומדדי רינדור (ערכם פג בדקות), ומטמוני המניפסט והזיהוי (נגזרים ובונים את עצמם). **קבצי עבודה** (`outputs/`, `uploads/`) קיבלו זמן שמירה בלי היסטוריה — `OUTPUT_RETENTION_HOURS` (48), נסחף בעלייה. נמחקו `data/mockups.db` (0 בתים) ו-`corners.json` (פלט של סקריפט מ-archive). שני באגים נתפסו תוך כדי: הסחיפה הראשונה מחקה `.gitkeep`, ורשימת הקבצים ה'תבועים' נשאבה מעמוד אחד של היסטוריה — מה שהיה מוחק קבצים חוקיים מעל 200 ייצואים. מתועד ב-`docs/what_is_stored.md`. | אין יותר מצב שבו ניקוי מטמון בדפדפן, restart או קריסה מאבדים משהו שאי אפשר לשחזר. |
