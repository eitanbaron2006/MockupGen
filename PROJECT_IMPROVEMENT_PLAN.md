...
**סטטוס בדיקות:** 192/192 טסטים עוברים בהצלחה + `ruff check` נקי  
...
| **P6.3** | ✅ **הושלם** | **חבילות ליסטינג אוטומטיות לחנויות (Etsy Listing Bundles)** | **מסך Listing sets** חדש: סט הוא שלושה אזורים ולא טבלה — **תמונה ראשית** אחת (רק מוקאפ MAIN), **עד 18 מוקאפים**, ו**מדריך גדלים** אחד. אין שורות, אין שמות פרטיים לתמונות (השם הוא שם המוקאפ) ואין תיבת סימון: האזור שנבחר קובע מה הפקד מציע — MAIN בלבד לתמונה הראשית, וכל השאר לשאר — כך שהכלל נשמר מעצם המבנה. המוקאפים שנבחרו מוצגים כתמונות. סוג המוצר הוא של החנות (Printable Wall Art, PNG Artwork Pack, Lightroom Presets, Digital Planners) ולא קטגוריות המוקאפים. **מדריכי גדלים**: מאגר שהאדמין מעלה או מייצר ב-Vertex מתוך המסך, מתויג ביחס שאומר גם את האוריינטציה (2:3 מול 3:2 הם שני צ'ארטים) — ורק כשאין התאמה במאגר, מיוצר אחד בזמן הרינדור ומסומן `source: ai`. 25 התבניות בקטגוריות MAIN שונו ל-`MAIN-…` והכלל נאכף אוטומטית ביצירה ובהעברה בין קטגוריות. `POST /api/mockups/listing-bundle` מקבל `set`; ללא סט נבחרת תבנית MAIN לתמונה הראשית ושתי תבניות שאינן MAIN אחריה. נמצא ותוקן באג: קובץ `.avif` נשמר בפועל כ-JPEG. 20 בדיקות חדשות; אומת בדפדפן מול הקטלוג האמיתי. | חיסכון של שעות עבודה ידניות בהכנת ליסטינגים, ושליטה מלאה במה שנכנס לכל ליסטינג. |
...
3. **נריץ את מערך הטסטים (192 בדיקות)** כדי לוודא שאין שום רגרסיה ושהכל עובד בצורה מושלמת.
