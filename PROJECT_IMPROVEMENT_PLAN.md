...
**סטטוס בדיקות:** 207/207 טסטים עוברים בהצלחה + `ruff check` נקי  
...
| **P4.2** | ⏳ בהמתנה | **שילוב Real-ESRGAN כ-AI Upscaler אוטומטי** | אינטגרציה של `RealESRGAN_x4plus.pth` להגדלה וחידוד של יצירות אמנות ברזולוציה נמוכה שהועלו על ידי לקוחות. **מוקפא לבקשת המשתמש (31.08.2026):** כבר קיימים סקריפטים עובדים תחת `archive/experiments`. המשתמש יעבור עליהם ויחליט מה למזג — ייתכן שזה יהפוך ליישום נפרד לגמרי. אין לבצע ללא הוראה מפורשת. | מוקאפים באיכות הדפסה חדה (300 DPI) גם מתמונות מקור קטנות. |
...
3. **נריץ את מערך הטסטים (207 בדיקות)** כדי לוודא שאין שום רגרסיה ושהכל עובד בצורה מושלמת.
